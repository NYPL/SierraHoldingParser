...
describe 'handler' do
    ...
    describe '#handle_event' do
        ...
        it "should invoke validate_record and send_record_to_stream for each record" do
            ...
            self.stubs(:validate_record).once.with(1).returns(@records[0])
            self.stubs(:validate_record).once.with(2).returns(@records[1])
            self.stubs(:validate_record).once.with(3).returns(@records[2])
            @mock_manager.stubs(:parse_record).times(3)
            self.stubs(:send_record_to_stream).times(3)
        ...
        it "should skip sending records to kinesis is avro validation fails" do
            self.stubs(:init).once
            self.stubs(:validate_record).once.with(1).returns(@records[0])
            self.stubs(:validate_record).once.with(2).raises(HoldingParserError.new)
            self.stubs(:validate_record).once.with(3).returns(@records[2])
            @mock_manager.stubs(:parse_record).twice
            self.stubs(:send_record_to_stream).once.with(@records[0])
            self.stubs(:send_record_to_stream).once.with(@records[2])
        ...
        it "should continue processing if sending to kinesis raises an error" do
            ...
            self.stubs(:validate_record).once.with(3).returns(@records[2])
            @mock_manager.stubs(:parse_record).times(3)
            self.stubs(:send_record_to_stream).once.raises(HoldingParserError.new)
            self.stubs(:send_record_to_stream).once.with(@records[1])
            self.stubs(:send_record_to_stream).once.with(@records[2])
    ...
    describe '#validate_record' do
        ...
        it "should raise an error if the record is missing the kinesis data object" do
            ...
            expect { self.send(:validate_record, {"other" => {"data" => "record"}}) }.to raise_error(HoldingParserError, "Unable to process incoming Kinesis record")
        end

        it "should raise an error if the decode method fails" do
            @mock_avro.stubs(:decode).raises(AvroError.new('testing'))

            expect { self.send(:validate_record, {"kinesis" => {"data" => "record"}}) }.to raise_error(HoldingParserError, "Incoming kinesis record failed Avro decoding")
        end
    end
